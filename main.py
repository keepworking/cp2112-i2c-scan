...
class HIDDriver:
    def __init__(self,*,serial=None, led=True):
        h = self.h = hid.Device(0x10C4, 0xEA90)
        ...
        self.h.send_feature_report(bytes([0x03, 0xFF, 0x00, 0x00, 0x00]))  # Set GPIO to Open-Drain
        for k in range(3):      # blinking LED
            print('.')
            self.h.send_feature_report(bytes([0x04, 0x00, 0xFF]))
            time.sleep(0.1)
            print('.')
            self.h.send_feature_report(bytes([0x04, 0xFF, 0xFF]))
            time.sleep(0.1)
        ...
        print("set gpio")
        self.h.send_feature_report(bytes([0x02, self.gpio_direction, self.gpio_pushpull, self.gpio_special, self.gpio_clockdiv]))  # initialize GPIO

        print("Set SMB Configuration (AN 495)")
        self.h.send_feature_report(bytes([0x06, 0x00, 0x06, 0x1A, 0x80, 0x02, 0x00, 0x00, 0xFF, 0x00, 0xFF, 0x01, 0x00, 0x0F]))
        #  0x00, 0x06, 0x1A, 0x80 <- SMBus at 400 kHz, the value, 0x61A80

    def I2CError(self):
        print("reset")
        self.h.send_feature_report(bytes([0x01, 0x01]))  # Reset Device for cancelling all transfers and reset configuration
        self.h.close()
    ...
    def write_byte(self, address, value):
        return self.h.write(bytes([0x14, address<<1, 0x01, value])) # Data Write Request

    def read_byte(self, address):
        self.h.write(bytes([0x10, address<<1, 0x00, 0x01])) # Data Read Request
    ...
    def write_byte_data(self, address, register, value):
        return self.h.write(bytes([0x14, address<<1, 0x02, register, value])) # Data Write Request

    def read_byte_data(self, address, register):
        self.h.write(bytes([0x11, address<<1, 0x00, 0x01, 0x01, register])) # Data Write Read Request
    ...
    def write_word_data(self, address, register, value):
        return self.h.write(bytes([0x14, address<<1, 0x03, register, value>>8, value & 0xFF])) # Word Write Request

    def read_word_data(self, address, register):
        self.h.write(bytes([0x11, address<<1, 0x00, 0x02, 0x01, register])) # Data Write Read Request
        self.h.write(bytes([0x12, 0x00, 0x02])) # Data Read Force
    ...
    def read_i2c_block(self, address, length):
        self.h.write(bytes([0x10, address<<1, 0x00, length])) # Data Read Request (60 bytes)
        ...
        for k in range(10):
            ...
            if (response[0] == 0x16) and (response[2] == 5):  # Polling a data
                ...
                self.h.write(bytes([0x12, response[5], response[6]])) # Data Read Force
